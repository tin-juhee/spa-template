...
import { COLOR_GROUPS, SHADES, DARK_SHADE_THRESHOLD } from '@/constants/designSystem';

type TTailwindColor = {
  [key: string]: { [shade: string]: string } | string;
};
    ...
    <table className='border-collapse table-auto w-full text-sm bg-gray-100'>
      <thead>
        <tr>
          ...
          {shades.map(shade => (
            <th key={shade} className='p-2 text-center font-semibold bg-gray-100'>
              {shade}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {colorGroups.map(group => (
          <tr key={group}>
            ...
            {shades.map(shade => {
              const bgClass = `bg-${group}-${shade}`;
              const colors = tailwindColors as TTailwindColor;
              const hexValue =
                typeof colors[group] === 'object'
                  ? (colors[group] as Record<string, string>)[shade]
                  : (colors[group] as string);

              const isDark = shade >= DARK_SHADE_THRESHOLD;
              const borderClass = isDark ? 'border-white/20' : 'border-black/20';
              return (
                <td key={`${group}-${shade}`} className='p-2 text-center'>
                  <div className={`h-8 w-8 rounded border ${bgClass} ${borderClass}`} title={hexValue ?? 'N/A'} />
                </td>
              );
            })}
          </tr>
